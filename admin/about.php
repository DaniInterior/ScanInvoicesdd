...
/**
 * \file    scaninvoices/admin/about.php
 * \ingroup scaninvoices
 * \brief   About page of module ScanInvoices.
 */
...
$langs->loadLangs(array("errors", "admin", "install", "scaninvoices@scaninvoices"));
...
if ($action == 'send_feedback') {
	...
	if ($rating > 0) {
		...
		if ($mail->sendfile()) {
			setEventMessage($langs->trans('ScanInvoicesFeedbackSent'), 'mesgs');
		} else {
			dol_syslog("ScanInvoices about.php: failed to send feedback email to ".$to, LOG_ERR);
			setEventMessage($langs->trans('ScanInvoicesFeedbackSendError'), 'errors');
		}
	} else {
		dol_syslog("ScanInvoices about.php: feedback submitted without rating", LOG_WARNING);
	}
...
$help_url = 'https://doc.cap-rel.fr/scaninvoices/';
$page_name = "ScanInvoicesAbout";

llxHeader('', $langs->trans($page_name), $help_url, '', 0, 0, [], ['/scaninvoices/css/admin.css.php']);
...
print dol_get_fiche_head($head, 'about', '', 0, 'scaninvoices@scaninvoices');
...
print '<h2>'.$langs->trans('ScanInvoicesDoYouLikeModule').'</h2>';
print '<p class="support-intro">'.$langs->trans('ScanInvoicesFeedbackIntro').'</p>';
...
print '<label>'.$langs->trans('ScanInvoicesYourRating').' :</label><br>';
...
print '<label for="feedback">'.$langs->trans('ScanInvoicesYourFeedback').' :</label><br>';
print '<textarea name="feedback" id="feedback" rows="6" class="flat minwidth400" placeholder="'.dol_escape_htmltag($langs->trans('ScanInvoicesFeedbackPlaceholder')).'"></textarea>';
print '</div>';

// Email
print '<div class="form-group">';
print '<label for="email">'.$langs->trans('ScanInvoicesYourEmail').' ('.$langs->trans('Optional').') :</label><br>';
...
print '<button type="submit" class="button" data-loading-text="'.dol_escape_htmltag($langs->trans('ScanInvoicesSendFeedback')).'">'.$langs->trans('ScanInvoicesSendFeedback').'</button>';
print '</form>';
...
print '<h2>&#9749; '.$langs->trans('ScanInvoicesBuyMeACoffee').'</h2>';
print '<p>'.$langs->trans('ScanInvoicesDonationText').'</p>';
...
print img_picto('', 'fa-coffee', 'class="pictofixedwidth"').' '.$langs->trans('ScanInvoicesOfferCoffee');
print '</a>';
print '</div>';
print '</div>';
...
print '<li><a href="https://doc.cap-rel.fr/scaninvoices/" target="_blank" rel="noopener noreferrer">'.img_picto('', 'fa-book').' '.$langs->trans('OnlineDocumentation').'</a></li>';
...
print '<li><a href="https://gitlab.com/cap-rel/scaninvoices" target="_blank" rel="noopener noreferrer">'.img_picto('', 'fa-code').' '.$langs->trans('ScanInvoicesSourceCode').'</a></li>';
print '</ul>';
print '</div>';
...
// Changelog section (read ChangeLog.md and parse line by line)
$changelog_path = dol_buildpath('/scaninvoices/ChangeLog.md', 0);
if (file_exists($changelog_path)) {
	...
	foreach ($lines as $line) {
		...
		if (strpos($trimmed, '### ') === 0) {
			...
			$html .= '<h4 class="si-cl-h4">'.dol_escape_htmltag(substr($trimmed, 4)).'</h4>';
		// Heading ## (version)
			...
			$html .= '<h3 class="si-cl-h3">'.dol_escape_htmltag(substr($trimmed, 3)).'</h3>';
		// Heading # (main title)
		} elseif (strpos($trimmed, '# ') === 0) {
			if ($inList) {
				$html .= '</ul>';
			...
			$html .= '<h3 class="si-cl-title">'.dol_escape_htmltag(substr($trimmed, 2)).'</h3>';
		// List item
	...
	print '<div class="si-changelog">'.$html.'</div>';
	print '</td></tr></table></div>';
}

// Submit-once protection on feedback form
...
print "            btn.dataset.originalText = btn.innerHTML;\n";
print "            btn.innerHTML = '... ' + (btn.dataset.loadingText || btn.textContent);\n";
print "        }\n";
print "    });\n";
print "});\n";
print '</script>'."\n";
