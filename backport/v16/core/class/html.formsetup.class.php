...
class FormSetup
{
	...
	public function saveConfFromPost($noMessageInUpdate = false)
	{
		...
		if (!$error) {
			...
			if (empty($noMessageInUpdate)) {
				setEventMessage($this->langs->trans("SetupSaved"));
			}
			return 1;
		} else {
			$this->db->rollback();
			if (empty($noMessageInUpdate)) {
				setEventMessage($this->langs->trans("SetupNotSaved"), 'errors');
			}
			return -1;
		}
	}
	...
	public function addItemFromParams($confKey, $params)
	{
		 ...
		 * 	// 'CAPCOMMENTS_MYPARAM1'=>array('type'=>'string', 'css'=>'minwidth500' ,'enabled'=>1),
			// 'CAPCOMMENTS_MYPARAM2'=>array('type'=>'textarea','enabled'=>1),
			//'CAPCOMMENTS_MYPARAM3'=>array('type'=>'category:'.Categorie::TYPE_CUSTOMER, 'enabled'=>1),
			//'CAPCOMMENTS_MYPARAM4'=>array('type'=>'emailtemplate:thirdparty', 'enabled'=>1),
			//'CAPCOMMENTS_MYPARAM5'=>array('type'=>'yesno', 'enabled'=>1),
			//'CAPCOMMENTS_MYPARAM5'=>array('type'=>'thirdparty_type', 'enabled'=>1),
			//'CAPCOMMENTS_MYPARAM6'=>array('type'=>'securekey', 'enabled'=>1),
			//'CAPCOMMENTS_MYPARAM7'=>array('type'=>'product', 'enabled'=>1),
		 */
