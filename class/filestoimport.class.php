...
/**
 * \file        class/filestoimport.class.php
 * \ingroup     scaninvoices
 * \brief       This file is a CRUD class file for Filestoimport (Create/Read/Update/Delete)
 */
...
class Filestoimport extends CommonObject
{
	 ...
	 */
	public $element = 'filestoimport';

	/**
	 * @var string Name of table without prefix where object is stored. This is also the key used for extrafields management.
	 */
	public $table_element = 'scaninvoices_filestoimport';
	...
	/**
	 * @var string String with name of icon for filestoimport. Must be the part after the 'object_' into object_filestoimport.png
	 */
	...
	//'1'=>'now', '2'=>'Later', '9'=>'Other'
	const QUEUE_NOW = 1;
	const QUEUE_LATER = 2;
	const QUEUE_MANUAL = 3;
	const QUEUE_OTHER = 9;

	// notes eric : ocr status
	// const STATUS_TOOMANYPAGES = -10;   //erreur: documen trop long
	// const STATUS_ERROR = -1;       //erreur
	...
	public $fields=array(
		...
		'message' => array('type'=>'html', 'label'=>'Message', 'enabled'=>'1', 'position'=>62, 'notnull'=>0, 'visible'=>3,),
		'date_creation' => array('type'=>'datetime', 'label'=>'DateCreation', 'enabled'=>'1', 'position'=>500, 'notnull'=>1, 'visible'=>4, 'csslist'=>'nowraponall'),
		'date_ocr_send' => array('type'=>'timestamp', 'label'=>'DateOcrSend', 'enabled'=>'1', 'position'=>501, 'notnull'=>0, 'visible'=>4, 'csslist'=>'nowraponall'),
		'date_ocr_return' => array('type'=>'timestamp', 'label'=>'DateOcrReturn', 'enabled'=>'1', 'position'=>502, 'notnull'=>0, 'visible'=>4, 'csslist'=>'nowraponall'),
	...
	//  */
	// public $table_element_line = 'scaninvoices_filestoimportline';

	// /**
	//  * @var string    Field with ID of parent key if this object has a parent
	//  */
	// public $fk_element = 'fk_filestoimport';

	// /**
	//  * @var string    Name of subtable class that manage subtable lines
	//  */
	// public $class_element_line = 'Filestoimportline';
	...
	//  */
	// protected $childtablesoncascade = array('scaninvoices_filestoimportdet');

	// /**
	//  * @var FilestoimportLine[]     Array of subtable lines
	//  */
	// public $lines = array();
	...
	public function __construct(DoliDB $db)
	{
		...
		/*if ($user->rights->scaninvoices->filestoimport->read) {
		...
		$this->status = Filestoimport::STATUS_DRAFT;
	}
	...
	{
		dol_syslog("ScanInvoices::FilesToImport: create status is : " . $this->status);
		if (null === $this->status) {
			dol_syslog("ScanInvoices::FilesToImport: create status is null -> set to Draft");
	...
	public function delete(User $user, $notrigger = false)
	{
		...
		$subpath = "now";
		if ($this->queue == Filestoimport::QUEUE_LATER) {
			$subpath = "later";
		}
		$completefilename = DOL_DATA_ROOT . '/scaninvoices/uploads/' . $subpath . '/' . $this->filename;
		if (file_exists($completefilename)) {
			@unlink($completefilename);
			@unlink(str_replace('.pdf', '.jpg', $completefilename));
		}
	...
	public function validate($user, $notrigger = 0)
	{
		...
		if (!empty($num)) {
			...
			if (!$error && !$notrigger) {
				// Call trigger
				$result = $this->call_trigger('FILESTOIMPORT_VALIDATE', $user);
				if ($result < 0) {
					$error++;
				}
				// End call triggers
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filename = CONCAT('".$this->db->escape($this->newref)."', SUBSTR(filename, ".(strlen($this->ref) + 1).")), filepath = 'filestoimport/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filename LIKE '".$this->db->escape($this->ref)."%' AND filepath = 'filestoimport/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				...
				$dirsource = $conf->scaninvoices->dir_output.'/filestoimport/'.$oldref;
				$dirdest = $conf->scaninvoices->dir_output.'/filestoimport/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->scaninvoices->dir_output.'/filestoimport/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function setDraft($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_DRAFT, $notrigger, 'FILESTOIMPORT_UNVALIDATE');
	}
	...
	public function cancel($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_CANCELED, $notrigger, 'FILESTOIMPORT_CANCEL');
	}
	...
	public function reopen($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_VALIDATED, $notrigger, 'FILESTOIMPORT_REOPEN');
	}
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$url = dol_buildpath('/scaninvoices/filestoimport_card.php', 1).'?id='.$this->id;
		...
		$hookmanager->initHooks(array('filestoimportdao'));
	...
	public function getLinesArray()
	{
		...
		$objectline = new FilestoimportLine($this->db);
		$result = $objectline->fetchAll('ASC', 'position', 0, 0, array('customsql'=>'fk_filestoimport = '.$this->id));
	...
	public function generateDocument($modele, $outputlangs, $hidedetails = 0, $hidedesc = 0, $hideref = 0, $moreparams = null)
	{
		...
		$modelpath = "core/modules/scaninvoices/doc/";
	...
	public function doScheduledJob()
	{
		...
		//Get files from network share
		$html = $this->getFilesFromNetworkShare();

		$this->db->begin();
		//0. Vérifier s'il y a des fichiers à traiter
		$allWaitingFiles = $this->fetchAll('DESC', 'date_creation', 10, 0, array('customsql' => " (t.status=" . self::STATUS_DRAFT . " OR t.status IS NULL) AND queue=" . self::QUEUE_LATER));

		dol_syslog("scaninvoices : file waiting liste " . json_encode($allWaitingFiles));

		if (count($allWaitingFiles) > 0) {
			//1. demander au serveur d'ocr s'il est libre
			$apiInfo = scaninvoicesApiGetInfoAboutWebservice('json');
			dol_syslog("scaninvoices : apiInfo " . json_encode($apiInfo));

			if (isset($apiInfo->load) &&  $apiInfo->load == 'readyForBackgroundTask') {
				$html .= $this->importInvoices($allWaitingFiles);
			} elseif (isset($apiInfo->load) &&  $apiInfo->load == 'pleaseSleepUntil') {
				if (isset($apiInfo->next_time)) {
					$nextTime = $apiInfo->next_time;
					$res=dolibarr_set_const($db, 'SCANINVOICES_CRON_NEXTRUN', $nextTime, 'chaine', 0, '', $conf->entity);
					// $job = new Cronjob($db);
					// if ($job->fetch(0, 'Filestoimport', 'doScheduledJob')) {
					// $job->datestart = dol_mktime(12, 0, 0, 2, 9, 2022);
					// // $job->datenextrun = dol_mktime(12, 0, 0, 2, 9, 2022);
					// // $job->reprogram_jobs($user->login, dol_mktime(12, 0, 0, 2, 9, 2022));
					// $job->update($user);
					// }
					dol_syslog("ScanInvoices llx_const : next time $nextTime");
				} else {
					dol_syslog("ScanInvoices llx_const : next time no next_time value");
				}
			} else {
				dol_syslog("ScanInvoices llx_const : no apiInfo->load data");
			}
		}
		$this->db->commit();
	...
	public function importInvoices($list)
	{
		...
		foreach ($list as $key => $fi) {
			...
			//TODO @ocr-server
			if (isset($apiInfo->load) &&  $apiInfo->load == 'readyForBackgroundTask') {
				$filetoimport = new Filestoimport($db);
				$filetoimport->fetch($fi->id);
				//validate -> in progress, take a ref number if needed
				$filetoimport->validate($user);
				$this->db->commit();

				//Note: il faudrait demander au serveur s'il est toujours ok, entre temps il a peut-être eu d'autres demandes :-)
				$basefilename = $filetoimport->filename;
				$completefilename = DOL_DATA_ROOT . '/scaninvoices/uploads/later/' . $basefilename;
				dol_syslog('ScanInvoices: importInvoices (later mode) : ' . $completefilename);
				$retour = scaninvoicesApiRunInvoiceAnalyze($filetoimport, $completefilename);
				dol_syslog('ScanInvoices: retour de  scaninvoicesApiRunInvoiceAnalyze : ' . json_encode($retour));

				if ($retour['error'] != "") {
					dol_syslog('ScanInvoices: Erreur de traitement, sauvegarde dans message (1)');
				}
				...
				if (substr($sapi_type, 0, 3) == 'cli') {
					$sleepTime = rand(5, 15);
					dol_syslog('ScanInvoices: importInvoices, sleep for : ' . $sleepTime);
					sleep($sleepTime);
				} else {
					dol_syslog('ScanInvoices: not from cli, import first file then return');
					return true;
				}
			} else {
				dol_syslog('ScanInvoices: OCR server is overloaded, please try again later');
				return true;
			}
		}
	}
	...
	public function importNow($fournID = -1)
	{
		...
		$this->date_ocr_send = dol_now();
		$this->validate($user);
		// $db->commit();

		$completefilename = DOL_DATA_ROOT . '/scaninvoices/uploads/' . $subpath . '/' . $this->filename;

		// dol_syslog("ScanInvoices:importNow :: " . json_encode($this));
		// dol_syslog("ScanInvoices:importNow with filename=" . $completefilename);
	...
	public function fileExist()
	{
		...
		$completefilename = DOL_DATA_ROOT . '/scaninvoices/uploads/' . $subpath . '/' . $this->filename;
		return(file_exists($completefilename));
	}
	...
	/**
	 * double check if that import is really a success (ie user did not delete invoice and forget to delete Filestoimport entry )
	 */
	public function fullImportSuccess()
	{
		...
		if ($this->status == self::STATUS_CLOSED) {
			...
			if (isset($this->fk_invoice)) {
				...
				if ($facfou->fetch($this->fk_invoice)) {
					return true;
				}
			}
		}
	...
	/**
	 * browse a network share(nextcloud) and get all files
	 */
	public function getFilesFromNetworkShare()
	{
		global $conf, $langs, $user, $db;
		...
		if (!empty($settings)) {
			...
			try {
				...
				$objLocalList = new Filestoimport($db);
				//Liste des fichiers déjà dans le "pipe" local à ne pas re-télécharger (on s'appuie uniquement sur le nom du fichier)
				$sql = "SELECT filename FROM ".MAIN_DB_PREFIX.$objLocalList->table_element;
				$resql = $db->query($sql);
				$localFilesList = [];
				while ($obj = $db->fetch_object($resql)) {
					$localFilesList[] = $obj->filename;
				}
				...
				foreach ($res as $filename => $value) {
					if (is_array($value) && isset($value['{DAV:}getcontenttype'])) {
						...
						$lastupdate = $value['{DAV:}getlastmodified'];

						//uniquement les PDF pour l'instant et si le fichier n'est pas déjà dans la base
						if ($contenttype == "application/pdf" && !in_array($basefilename, $localFilesList)) {
							dol_syslog("ScanInvoices : Plan to import a new file : " . $basefilename);
							$html .= "<li> Import file " . $basefilename;
							$tmpfname = tempnam(DOL_DATA_ROOT.'/scaninvoices/temp/', $basefilename);

							$fp = fopen($tmpfname, "w");
							if ($fp) {
								$download = $client->request('GET', $filename);
								fwrite($fp, $download['body']);
								fclose($fp);
								$sha1 = sha1_file($tmpfname);

								//download file, store local, insert into queue "later" if not already present
								...
								/** @phpstan-ignore-next-line */
								if (dol_move($tmpfname, $completefilename, 0, 1, 0, 0) <= 0) {
									dol_syslog("ScanInvoices : error importing " . $basefilename . " fail to save file");
									$html .= " error, failed to save uploaded file '".$tmpfname."'. Please check permissions or file size limits.";
									break;
								} else {
									dol_syslog("ScanInvoices : " . $basefilename . " downloaded");
									$html .= ", download ok";
									...
									if ($objectid <= 0) {
										$db->rollback();
										$html .= " error, failed database access : ".$object->error;
										dol_syslog("ScanInvoices : " . $basefilename . " error database access : " . $object->error);
									} else {
										$html .= ", save to database success !</li>";
										dol_syslog("ScanInvoices : " . $basefilename . " pushed to import queue success !");
										$db->commit();
									}
									$row['id'] = $objectid;
								}
							} else {
								dol_syslog("ScanInvoices : " . $basefilename . " fail to write file !");
								$html .= " error, fail to write file";
							}
						}
					}
				}
			} catch (Exception $e) {
				dol_syslog("ScanInvoices : generic error on network access : " . $e->getMessage());
...
/**
 * Class FilestoimportLine. You can also remove this and generate a CRUD class for lines objects.
 */
class FilestoimportLine extends CommonObjectLine
{
	// To complete with content of an object FilestoimportLine
	// We should have a field rowid, fk_filestoimport and position
