...
$contextpage = GETPOST('contextpage', 'aZ') ?GETPOST('contextpage', 'aZ') : 'settingscard'; // To manage different context of search
...
$hookmanager->initHooks(array('settingscard', 'globalcard')); // Note that conf->hooks_modules contains array
...
if (empty($reshook)) {
	...
	$backurlforlist = dol_buildpath('/scaninvoices/settings_list.php', 1);
	...
	if (empty($backtopage) || ($cancel && empty($id))) {
		if (empty($backtopage) || ($cancel && strpos($backtopage, '__ID__'))) {
			if (empty($id) && (($action != 'add' && $action != 'create') || $cancel)) {
				...
				$backtopage = dol_buildpath('/scaninvoices/settings_card.php', 1).'?id='.($id > 0 ? $id : '__ID__');
			}
		}
	}

	$triggermodname = 'SCANINVOICES_SETTINGS_MODIFY'; // Name of trigger action code to execute when we modify record
	...
	$triggersendname = 'SCANINVOICES_SETTINGS_SENTBYMAIL';
	$autocopy = 'MAIN_MAIL_AUTOCOPY_SETTINGS_TO';
	$trackid = 'settings'.$object->id;
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create'))) {
	...
	$head = settingsPrepareHead($object);
	print dol_get_fiche_head($head, 'card', $langs->trans("Settings"), -1, $object->picto);

	$formconfirm = '';

	// Confirmation to delete
	if ($action == 'delete') {
		$formconfirm = $form->formconfirm($_SERVER["PHP_SELF"].'?id='.$object->id, $langs->trans('DeleteSettings'), $langs->trans('ConfirmDeleteObject'), 'confirm_delete', '', 0, 1);
	}
	...
	if ($action != 'presend' && $action != 'editline') {
		...
		if (empty($reshook)) {
			...
			*/
			...
			print dolGetButtonAction($langs->trans('Delete'), '', 'delete', $_SERVER['PHP_SELF'].'?id='.$object->id.'&action=delete', '', $permtodelete);
		}
		print '</div>'."\n";
	}
	...
	if ($action != 'presend') {
		...
		if ($includedocgeneration) {
			...
			print $formfile->showdocuments('scaninvoices:Settings', $object->element.'/'.$objref, $filedir, $urlsource, $genallowed, $delallowed, $object->model_pdf, 1, 0, 0, 28, 0, '', '', '', $langs->defaultlang);
		}

		// Show links to link elements
		$linktoelem = $form->showLinkToObjectBlock($object, null, array('settings'));
		...
		$morehtmlright = '<a href="'.dol_buildpath('/scaninvoices/settings_agenda.php', 1).'?id='.$object->id.'">';
