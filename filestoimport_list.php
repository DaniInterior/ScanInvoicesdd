...
/**
 *   	\file       filestoimport_list.php
 *		\ingroup    scaninvoices
 *		\brief      List page for filestoimport
 */
...
$contextpage = GETPOST('contextpage', 'aZ') ? GETPOST('contextpage', 'aZ') : 'filestoimportlist'; // To manage different context of search
...
$hookmanager->initHooks(array('filestoimportlist')); // Note that conf->hooks_modules contains array
...
//$result = restrictedArea($user, 'scaninvoices', $id, '');
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'Filestoimport';
	$objectlabel = 'Filestoimport';
	$uploaddir = $conf->scaninvoices->dir_output;
	include DOL_DOCUMENT_ROOT.'/core/actions_massactions.inc.php';

	if ($massaction == "analyze") {
		$importHtml = "<div class='div-table-responsive-no-min'><table class='ScanInvoicesStyled-table'>";
		$importHtml .= "<thead>";
		...
		dol_syslog("ScanInvoice : Import Now returns " . json_encode($res));
	}
}
...
//$help_url="EN:Module_Filestoimport|FR:Module_Filestoimport_FR|ES:Módulo_Filestoimport";
$help_url = '';
$title = $langs->trans("Filestoimports");
$morejs = array();
$morecss = array();
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/scaninvoices/importauto.php', 1), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, 'object_'.$object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendFilestoimportRef";
$modelmail = "filestoimport";
$objecttmp = new Filestoimport($db);
...
if ($search_all) {
	...
	print '<!-- Search done like if FILESTOIMPORT_QUICKSEARCH_ON_FIELDS = '.$setupstring.' -->'."\n";
...
// 	print $formfile->showdocuments('massfilesarea_scaninvoices', '', $filedir, $urlsource, 0, $delallowed, '', 1, 1, 0, 48, 1, $param, $title, '', '', '', null, $hidegeneratedfilelistifempty);
// }
