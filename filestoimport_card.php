...
/**
 *   	\file       filestoimport_card.php
 *		\ingroup    scaninvoices
 *		\brief      Page to create/edit/view filestoimport
 */
...
$langs->loadLangs(array("scaninvoices@scaninvoices", "other"));
...
$contextpage = GETPOST('contextpage', 'aZ') ?GETPOST('contextpage', 'aZ') : 'filestoimportcard'; // To manage different context of search
...
$diroutputmassaction = $conf->scaninvoices->dir_output.'/temp/massgeneration/'.$user->id;
$hookmanager->initHooks(array('filestoimportcard', 'globalcard')); // Note that conf->hooks_modules contains array
...
$result = restrictedArea($user, 'scaninvoices', $object->id, 'scaninvoices_filestoimport', '', 'fk_soc', 'rowid', $isdraft);
...
if (empty($reshook)) {
	...
	$backurlforlist = dol_buildpath('/scaninvoices/filestoimport_list.php', 1);
	...
	if (empty($backtopage) || ($cancel && empty($id))) {
		if (empty($backtopage) || ($cancel && strpos($backtopage, '__ID__'))) {
			if (empty($id) && (($action != 'add' && $action != 'create') || $cancel)) {
				...
				$backtopage = dol_buildpath('/scaninvoices/filestoimport_card.php', 1).'?id='.($id > 0 ? $id : '__ID__');
			}
		}
	}

	$triggermodname = 'SCANINVOICES_FILESTOIMPORT_MODIFY'; // Name of trigger action code to execute when we modify record
	...
	$trackid = 'filestoimport'.$object->id;
...
$title = $langs->trans("Filestoimport");
$help_url = '';
...
// Part to create
if ($action == 'create') {
	print load_fiche_titre($langs->trans("NewObject", $langs->transnoentitiesnoconv("Filestoimport")), '', 'object_'.$object->picto);
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create'))) {
	...
	$head = filestoimportPrepareHead($object);
	print dol_get_fiche_head($head, 'card', $langs->trans("Filestoimport"), -1, $object->picto);

	$formconfirm = '';

	// Confirmation to delete
	if ($action == 'delete') {
		$formconfirm = $form->formconfirm($_SERVER["PHP_SELF"].'?id='.$object->id, $langs->trans('DeleteFilestoimport'), $langs->trans('ConfirmDeleteObject'), 'confirm_delete', '', 0, 1);
	}
	...
	// Object card
	// ------------------------------------------------------------
	$linkback = '<a href="'.dol_buildpath('/scaninvoices/filestoimport_list.php', 1).'?restore_lastsearch_values=1'.(!empty($socid) ? '&socid='.$socid : '').'">'.$langs->trans("BackToList").'</a>';
	...
	print '<div class="divforapijpgfile">';
	print '<img class="apijpgfile" style="width: inherit;" src="api.php/jpgfile/' . $object->filename . '&token=' . currentToken() . '">';
	print '</div>';
	print '</td></tr>';
	...
	if ($action != 'presend' && $action != 'editline') {
		...
		if (empty($reshook)) {
			...
			if ($object->status == $object::STATUS_HALFSUCCESS || $object->status == $object::STATUS_ERROR || $object->status == $object::STATUS_DRAFT) {
				$target = dol_buildpath("/scaninvoices/importinvoice.php", 1)."?filenamePDF=" . urlencode($object->filename).'&action=create';
				$target .= "&filestoimportId=" . $object->id;
	...
	if ($action != 'presend') {
		...
		$linktoelem = $form->showLinkToObjectBlock($object, null, array('filestoimport'));
		...
		$morehtmlright = '<a href="'.dol_buildpath('/scaninvoices/filestoimport_agenda.php', 1).'?id='.$object->id.'">';
	...
	// Presend form
	$modelmail = 'filestoimport';
	$defaulttopic = 'InformationMessage';
	$diroutput = $conf->scaninvoices->dir_output;
	$trackid = 'filestoimport'.$object->id;
